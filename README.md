...
| Invitation links | <https://runit-app.pages.dev> — Cloudflare Pages, output dir `web/`, deployed from this repo. It exists because Apple fetches the association file from the domain ROOT and GitHub Pages can neither serve the apex here nor set `Content-Type: application/json`. See `web/README.md`. **Deployed 2026-09-07 by DIRECT UPLOAD** (`wrangler pages deploy web --project-name=runit-app`), so a push to `main` does not redeploy it -- re-run that command when `web/` changes. **Still unverified on a device** (#54): a misconfigured universal link fails silently into Safari, and only Settings -> Developer -> Universal Links -> Diagnostics on a real iPhone settles it. The name `runit` was taken by an unrelated project — `pnpm verify:links` is what catches that class of mistake. |
